...
for($i = 0; $i < count($_POST['chk']); $i++){
    ...
    //找出特定 itemId 的資料
    $sqlImg = "SELECT `courseImg` FROM `course` WHERE `courseId` = ? ";
    ...
    if($stmt_img->rowCount() > 0) {
        ...
        //刪除檔案
        $bool = unlink("../images/course/".$arr[0]['courseImg']);
...
if($count > 0) {
    ...
    ?><script>alert("刪除成功! 回到課程列表")</script><?php 
    // echo json_encode($objResponse, JSON_UNESCAPED_UNICODE);
    exit();
} else {
    header("Refresh: 3; url=./admin.course.search.php");
