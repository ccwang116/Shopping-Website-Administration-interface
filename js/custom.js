$(document).ready(function(){
    ...
    //購物車商品數量增加
    $(document).on("click", "p#increase", function(e){
        let p = $(this);
        $.ajax({
            method: "POST",
            url: "./increaseQty.php",
            dataType: "json",
            data: { 
                itemId: p.attr('data-item-id'), 
                cartQty: 1
            }
        })
        .done(function( json ) {
            $("p#cartQty"+json.cartQtyid).text(json.cartQty);
            $("p#subtotal"+json.cartQtyid).text(json.subtotal);
            $("mark#total").text(json.total);
            $("mark#shipTotal").text(json.shipTotal);
        })
        .fail(function( jqXHR, textStatus ) {
            alert( "Request failed: " + textStatus );
        });
    ...
    //購物車商品數量減少
    $(document).on("click", "p#decrease", function(e){
        let p = $(this);
        $.ajax({
            method: "POST",
            url: "./decreaseQty.php",
